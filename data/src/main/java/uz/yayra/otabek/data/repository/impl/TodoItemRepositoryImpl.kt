...
class TodoItemRepositoryImpl @Inject constructor(
    ...
    override suspend fun getTodo(network: Boolean, isShow: Boolean): Result<List<TodoEntity>> = withContext(Dispatchers.IO) {
        if (network) {
                ...
                val response = api.add(
                    ToDoRequest.AddToDo(
                        element = ToDoRequest.TodoItemAdd(
                            it.id.toString(), it.text, it.importance, it.deadLine, it.isCompleted, "#FFFFFF", it.createdAt, it.modifiedAt, "Samsung Galaxy A52"
                        )
                    )
                )
                if (response.isSuccessful && response.body() != null) {
